...
class BioTests(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        User.objects.create_user('temporary', 'temporary@temporary.com', 'temporary')
    ...
    def test_bio_in_the_context(self):
         ...
         with open(os.path.join(settings.MEDIA_ROOT, "foto.png"),'rb') as img_file_read:
             image_file = ImageFile(img_file_read.read())
             MyBio.objects.create(first_name='Oleksii', last_name='Aledinov', photo=image_file)
             response = self.client.get('/')
    ...
    def test_bio_edit(self):
         with open(os.path.join(settings.MEDIA_ROOT, "foto.png"),'rb') as img_file_read:
            image_file = ImageFile(img_file_read.read())
            bio_object = MyBio.objects.create(first_name='Oleksii', last_name='Aledinov', photo=image_file)
            response = self.client.get('/edit/{0}/'.format(bio_object.id))
            self.assertEquals(response.context['bio'].first_name, bio_object.first_name)
